    ...
    start() {
        modbus.tcp.server({debug: this.options.dev}, conn => {
            ...
            core.getCoreID().then(id => {
                ...
                global.event.publish(false, 'modbus/status', null, 'online', 30, moment(new Date()).toISOString(), coreID);
            ...
            core.on('disconnect', d => {
                ...
                // 设置当前设备为离线状态
                this._attribsByID[coreID].connected = false
                core = null
                global.event.publish(false, 'modbus/status', null, 'offline', 30, moment(new Date()).toISOString(), coreID);
